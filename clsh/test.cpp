...
char* get_port(char* name) {
    if (strcmp(name, "work01")==0)
        return "2223";
    else if (strcmp(name, "work02")==0)
        return "2224";
    else if (strcmp(name, "work03")==0)
        return "2225";
}
...
void child(int child_idx) {
    ...
    char* port = get_port(opt.nodes[child_idx]);
    //printf("%s %s\n", opt.nodes[child_idx], port);
    fflush(0);
    if (execlp("ssh", "ssh", "-T", "-i", "id_rsa", "ubuntu@localhost", "-p", port, NULL) == -1) {
        perror("execlp");
        exit(EXIT_FAILURE);
    }
}
